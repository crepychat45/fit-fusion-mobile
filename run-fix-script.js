...
function fixAllImports() {
  ...
  function replaceInFile(filePath) {
    ...
    const oldToastImport = /import\s*{\s*useToast\s*}\s*from\s*["']@\/components\/ui\/use-toast["'];?/g;
    if (oldToastImport.test(content)) {
      content = content.replace(oldToastImport, `import { useToast } from "@/hooks/use-toast";`);
      changed = true;
    }
    
    // Fix missing key props in map functions - improved regex
    const mapKeyRegex = /\.map\s*\(\s*\(([^)]+)\)\s*=>\s*<([^>\s]+)(?![^>]*\skey=)/g;
    content = content.replace(mapKeyRegex, (match, params, element) => {
      const keyProp = params.includes(',') ? params.split(',')[1].trim() : 'index';
      const replacement = match.replace('<' + element, `<${element} key={${keyProp}}`);
      changed = true;
      return replacement;
    });
    ...
    content = content.replace(fragmentRegex, (match, jsxContent) => {
      if (jsxContent.match(/<[^>]+>\s*<[^>]+>/)) {
        changed = true;
        return match.replace('return (', 'return (\n    <React.Fragment>').replace(jsxContent, `${jsxContent}\n    </React.Fragment>`);
      }
      return match;
    });
