...
export const EnhancedSettingsProvider = ({ children }: SettingsProviderProps) => {
  ...
  const [settingsBackup, setSettingsBackup] = useState<Record<string, any> | null>(null);
  ...
  // Export settings
  const [exportFormat, setExportFormatState] = useState<"json" | "csv" | "pdf" | "html">(() => {
  ...
  const persistSetting = useCallback((key: string, value: any) => {
    try {
  ...
  const setExportFormat = useCallback((format: "json" | "csv" | "pdf" | "html") => {
    setExportFormatState(format);
    persistSetting("fitfusion-export-format", format);
  }, [persistSetting]);

  const setExportAnonymized = useCallback((anonymized: boolean) => {
    setExportAnonymizedState(anonymized);
    persistSetting("fitfusion-export-anonymized", anonymized);
  }, [persistSetting]);

  const setExportCategories = useCallback((categories: string[]) => {
  ...
  const setAutoSync = useCallback((sync: boolean) => {
    setAutoSyncState(sync);
    persistSetting("fitfusion-auto-sync", sync);
  }, [persistSetting]);

  const setCloudBackup = useCallback((backup: boolean) => {
  ...
  const setCCodeEditorEnabled = useCallback((enabled: boolean) => {
  ...
  const removeCustomSound = useCallback((name: string) => {
  ...
  const addProgrammingLanguage = useCallback((language: string) => {
  ...
  const removeProgrammingLanguage = useCallback((language: string) => {
    const newLanguages = programmingLanguages.filter(lang => lang !== language);
    setProgrammingLanguages(newLanguages);
    persistSetting("fitfusion-programming-languages", newLanguages);
  }, [programmingLanguages, persistSetting]);
  
  // Functions for developer options
  const setDeveloperOption = useCallback((option: keyof typeof developerOptions, value: boolean) => {
  ...
  const setDisplayOption = useCallback((option: keyof typeof displayOptions, value: boolean) => {
  ...
  const saveProfileInfo = useCallback(async (profileData: Record<string, any>): Promise<boolean> => {
    try {
  ...
  const resetAllSettings = useCallback(async (): Promise<boolean> => {
    try {
      ...
      const keysToRemove = Object.keys(localStorage).filter(key => key.startsWith('fitfusion-'));
  ...
  const exportSettings = useCallback(async (): Promise<string> => {
    ...
    soundEnabled, soundVolume, hapticEnabled, hapticFeedback, soundPack, customSounds,
    compactView, showCalories, showHeartRate, unitSystem, programmingLanguages,
    exportFormat, exportAnonymized, exportCategories, theme, fontSize, language,
    autoSync, cloudBackup, notifications, codeEditorEnabled, appVersion,
    subscriptionPlan, paymentMethod, developerOptions, displayOptions, toast
  ]);
  ...
  const importSettings = useCallback(async (settingsData: string): Promise<boolean> => {
    try {
      ...
      const confirmImport = window.confirm("Are you sure you want to import these settings? This will overwrite your current settings.");
      if (!confirmImport) return false;

      // Import each setting with validation
      if (typeof parsedSettings.soundEnabled === 'boolean') setSoundEnabled(parsedSettings.soundEnabled);
      if (typeof parsedSettings.soundVolume === 'number') setSoundVolume(parsedSettings.soundVolume);
      if (typeof parsedSettings.hapticEnabled === 'boolean') setHapticEnabled(parsedSettings.hapticEnabled);
      if (typeof parsedSettings.hapticFeedback === 'boolean') setHapticFeedback(parsedSettings.hapticFeedback);
      if (typeof parsedSettings.soundPack === 'string') setSoundPack(parsedSettings.soundPack);
      if (parsedSettings.customSounds && typeof parsedSettings.customSounds === 'object') {
        setCustomSounds(parsedSettings.customSounds);
      }
      if (typeof parsedSettings.compactView === 'boolean') setCompactView(parsedSettings.compactView);
      if (typeof parsedSettings.showCalories === 'boolean') setShowCalories(parsedSettings.showCalories);
      if (typeof parsedSettings.showHeartRate === 'boolean') setShowHeartRate(parsedSettings.showHeartRate);
      ...
      if (typeof parsedSettings.exportAnonymized === 'boolean') setExportAnonymized(parsedSettings.exportAnonymized);
      if (Array.isArray(parsedSettings.exportCategories)) setExportCategories(parsedSettings.exportCategories);
      ...
      if (typeof parsedSettings.autoSync === 'boolean') setAutoSync(parsedSettings.autoSync);
      if (typeof parsedSettings.cloudBackup === 'boolean') setCloudBackup(parsedSettings.cloudBackup);
      if (typeof parsedSettings.notifications === 'boolean') setNotifications(parsedSettings.notifications);
      if (typeof parsedSettings.codeEditorEnabled === 'boolean') setCCodeEditorEnabled(parsedSettings.codeEditorEnabled);
      if (parsedSettings.subscriptionPlan) setSubscriptionPlan(parsedSettings.subscriptionPlan);
      if (parsedSettings.paymentMethod) setPaymentMethod(parsedSettings.paymentMethod);
      
      if (parsedSettings.developerOptions && typeof parsedSettings.developerOptions === 'object') {
        setDeveloperOptions(parsedSettings.developerOptions);
      }
      
      if (parsedSettings.displayOptions && typeof parsedSettings.displayOptions === 'object') {
    ...
    setSoundEnabled, setSoundVolume, setHapticEnabled, setHapticFeedback, setSoundPack,
    setCompactView, setShowCalories, setShowHeartRate, setUnitSystem, setExportFormat,
    setExportAnonymized, setExportCategories, setTheme, setFontSize, setLanguage,
    setAutoSync, setCloudBackup, setNotifications, setCCodeEditorEnabled,
    setSubscriptionPlan, setPaymentMethod, toast
  ]);
  ...
  const createBackup = useCallback(() => {
    const backup = {
      ...
      exportFormat, exportAnonymized, exportCategories, theme, fontSize, language,
      autoSync, cloudBackup, notifications, codeEditorEnabled, appVersion,
      subscriptionPlan, paymentMethod, developerOptions, displayOptions,
  ...
  const restoreBackup = useCallback(() => {
    ...
    const confirmRestore = window.confirm("Are you sure you want to restore from backup? This will overwrite your current settings.");
