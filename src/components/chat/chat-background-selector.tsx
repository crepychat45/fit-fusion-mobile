...
export function ChatBackgroundSelector({ 
  ...
  const backgrounds = [
    ...
    },
    {
      ...
      style: 'bg-white',
      preview: '#ffffff',
      pattern: 'repeating-linear-gradient(45deg, transparent, transparent 10px, #f3f4f6 10px, #f3f4f6 20px)',
      patternSize: 'auto'
    }
  ];
  ...
  return (
    <div className="w-96 bg-background border rounded-lg shadow-lg p-4">
      <div className="mb-4">
        ...
        <p className="text-sm text-muted-foreground">Customize your chat appearance</p>
      </div>
        ...
        {backgrounds.map((bg) => (
          <Card 
            ...
            className={`relative p-3 cursor-pointer transition-all hover:shadow-md ${
              currentBackground.includes(bg.style.split(' ')[0]) ? 'ring-2 ring-primary' : ''
            }`}
