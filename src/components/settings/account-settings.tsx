...
import { User, Mail, Key, Lock, LogOut, Smartphone, Laptop, RefreshCcw } from "lucide-react";
...
export function AccountSettings() {
  ...
  const handleSignUp = async () => {
    if (!email || !password || password !== confirmPassword) {
      toast({
        title: "Invalid Input",
        description: password !== confirmPassword ? "Passwords don't match" : "Please fill out all fields",
        variant: "destructive",
      });
      return;
    }
  ...
  const handlePasswordChange = async () => {
    if (!password || !newPassword || newPassword !== confirmPassword) {
      toast({
        title: "Invalid Input",
        description: newPassword !== confirmPassword ? "New passwords don't match" : "Please fill out all fields",
        variant: "destructive",
      });
      return;
    }
      ...
      {!isAuthenticated ? (
        <Card className="liquid-glass border-border/30">
          ...
          <CardFooter className="flex flex-col gap-2 sm:flex-row">
            <Button className="w-full sm:w-auto" onClick={handleLogin} disabled={isLoading}>
              {isLoading ? "Signing in..." : "Sign In"}
            </Button>
            <Button variant="outline" className="w-full sm:w-auto" onClick={handleSignUp} disabled={isLoading}>
              ...
              <div className="space-y-4">
                {connectedDevices.map((device) => (
                  ...
                  >
                    ...
                    {!device.isCurrentDevice && (
                      <Button variant="ghost" size="sm" onClick={() => handleDisconnectDevice(device.id)}>
                        Disconnect
                      </Button>
                    )}
                  </div>
      ...
      <Dialog open={logoutDialogOpen} onOpenChange={setLogoutDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Are you sure you want to sign out from all devices?</DialogDescription>
          </DialogHeader>
