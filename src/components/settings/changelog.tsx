...
interface ChangelogEntry {
  ...
  changes: {
    category: 'feature' | 'fix' | 'security' | 'performance' | 'ui' | 'breaking';
    description: string;
    important?: boolean;
  }[];
  downloadUrl?: string;
...
const changelog: ChangelogEntry[] = [
  {
    ...
    changes: [
      { category: "feature", description: "🎉 Major UI redesign with improved navigation and modern design language", important: true },
      { category: "fix", description: "🔧 Fixed update installation and version persistence issues", important: true },
      { category: "feature", description: "📊 Enhanced dashboard with real-time analytics and progress tracking" },
      { category: "security", description: "🔒 Advanced security features and enhanced privacy controls", important: true },
      { category: "performance", description: "⚡ 60% performance improvement in load times and app responsiveness", important: true },
      { category: "feature", description: "🎯 Smart workout recommendations powered by AI technology" },
      { category: "ui", description: "📱 Better mobile responsiveness across all devices and screen sizes" },
      { category: "feature", description: "🏆 New achievement system with progress milestones" },
      { category: "fix", description: "🐛 Resolved settings menu visibility and button functionality issues" },
      { category: "feature", description: "💬 Enhanced chat interface with improved messaging features" }
    ],
    downloadUrl: "#",
    size: "18.7 MB"
  },
...
export function Changelog() {
  const [expandedVersions, setExpandedVersions] = useState<Set<string>>(new Set(['5.0.2']));
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-4">
          {filteredChangelog.map((entry) => {
            ...
            return (
              ...
              >
                <Card className={`overflow-hidden ${isCurrentVersion ? 'ring-2 ring-blue-500 bg-blue-50 dark:bg-blue-950/20' : ''}`}>
                  <CardHeader 
                            ...
                            {entry.changes.map((change, index) => (
                              <div key={index} className="flex items-start gap-3">
                                <div className={`p-1 rounded-full ${
                                  change.category === 'feature' ? 'bg-blue-100 text-blue-600 dark:bg-blue-900 dark:text-blue-300' :
                                  change.category === 'fix' ? 'bg-green-100 text-green-600 dark:bg-green-900 dark:text-green-300' :
                                  change.category === 'security' ? 'bg-red-100 text-red-600 dark:bg-red-900 dark:text-red-300' :
                                  change.category === 'performance' ? 'bg-purple-100 text-purple-600 dark:bg-purple-900 dark:text-purple-300' :
                                  change.category === 'ui' ? 'bg-orange-100 text-orange-600 dark:bg-orange-900 dark:text-orange-300' :
                                ...
                                <div className="flex-1">
                                  <div className="flex items-center gap-2">
                                    ...
                                    {change.important && (
                                      <Badge variant="default" className="text-xs px-1 py-0 bg-amber-500">
                                        Important
                                      </Badge>
                                    )}
                                  </div>
      ...
      <div className="text-center text-sm text-muted-foreground">
        ...
        <p className="mt-1">🔄 Updates are automatically checked when enabled</p>
      </div>
    </div>
  );
}
