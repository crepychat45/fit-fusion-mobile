...
export type SoundEffect = "workout-start" | "workout-complete" | "notification" | "achievement" | "error" | "success" | "tap";
...
export const playSound = (sound: SoundEffect, volume = 1.0): Promise<void> => {
  return new Promise((resolve, reject) => {
    try {
      ...
      const soundEnabled = localStorage.getItem('fitfusion-sound-enabled') !== 'false';
      if (!soundEnabled) {
        resolve();
        return;
      }
      ...
      const actualVolume = storedVolume ? (parseInt(storedVolume) / 100) * volume : volume;
...
// Vibration patterns
export type VibrationPattern = "short" | "medium" | "long" | "double" | "success" | "error" | "warning";
...
export const vibrate = (pattern: VibrationPattern = "short"): void => {
  try {
    ...
    const hapticEnabled = localStorage.getItem('fitfusion-haptic-enabled') !== 'false';
